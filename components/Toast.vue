...
<template>
  <li :class="containerClasses"
    class="pointer-events-auto cursor-default relative py-2 px-3 overflow-clip rounded-xl rounded-tl-none border w-fit max-w-80 backdrop-blur-xl"
    @click="emit('removeToast', props.id)">
    <span>{{ props.message }}</span>
    <div class="absolute-center -z-10">
      <ClipPath />
    </div>
    <div :class="progressionContainerClasses" class="absolute bottom-0 w-full left-0 h-0.5">
      <div ref="progression" :class="progressionClasses" class="h-0.5 rounded-full duration-75" style="width: 0;"></div>
    </div>
  </li>
</template>
